...
const CadUser = () => {
  ...
  const handleCadastro = async (e) => {
    ...
    try {
      ...
      const checkResponse = await api.get('/usuarios');
      ...
      if (userExists) {
        setError('Usuário já cadastrado (nome, CPF ou email duplicado)');
        setLoading(false);
        return;
      }
      ...
      if (error.response) {
        ...
        setError(`Erro ${error.response.status}: ${error.response.data.message || 'Falha no cadastro'}`);
